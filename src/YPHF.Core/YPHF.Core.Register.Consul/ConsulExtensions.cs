...
namespace YPHF.Core.Register.Consul
{
    ...
    public static class ConsulExtensions
    {
        ...
        public static IServiceCollection AddConsul(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                throw new Exception("Appsettings Must Be Register");
            }
        ...
        public static async Task<IApplicationBuilder> UseConsulAsync(this WebApplication app)
        {
            ...
            {
                throw new Exception("Appsettings Must Be ServiceConfig.Name");
            }

            var flagServicePort = int.TryParse(configuration["Service:Port"], out var servicePort);

            if (!flagServicePort || servicePort < 1)
            {
                throw new Exception("Appsettings Must Be ServiceConfig.Port");
            }
            ...
            {
                throw new Exception("Appsettings Must Be ServiceConfig.Address");
            }

            var flagTimeout = int.TryParse(configuration["Service:Timeout"], out var timeout);

            if (!flagTimeout || timeout < 1)
            {
                timeout = 5;
            }
            ...
            if (!flagDeregister || deregister < 1)
            {
                deregister = 30;
            }

            _ = bool.TryParse(configuration["Service:Https"], out bool iShttps);
            ...
            if (check == Const.Check)
            {
                app.MapGet(check, () => new BaseResult());
            }
