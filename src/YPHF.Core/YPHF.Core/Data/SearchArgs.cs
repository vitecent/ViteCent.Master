...
namespace YPHF.Core.Data
{
    ...
    public class SearchArgs : BaseArgs
    {
        ...
        public (string, object) ToSQL()
        {
            ...
            //删除空
            Args.RemoveAll(x => string.IsNullOrWhiteSpace(x.Field) || string.IsNullOrWhiteSpace(x.Value?.ToString()) || x.Value?.ToString() == "[]");
            ...
            if (list.Count > 1)
            {
                var keys = list.Select(x => x.Group.Split(',', StringSplitOptions.RemoveEmptyEntries)[0]).Distinct().ToList();
