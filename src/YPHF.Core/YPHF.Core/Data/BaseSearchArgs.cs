...
namespace YPHF.Core.Data
{
    ...
    public static class BaseSearchArgs
    {
        ...
        public static void AddArgs(this SearchArgs args, string field, object value, SearchEnum method = SearchEnum.Equal, string group = "")
        {
            if (args.Args.Any(x => x.Field == field))
            {
                args.Args.RemoveAll(x => x.Field == field);
            }
        ...
        public static void AddConmpanyId(this SearchArgs args, BaseUserInfo user, string field = "CompanyId")
        {
            ...
            {
                args.AddArgs(field, user?.Company?.Id ?? "");
            }
        }
        ...
        {
            if (args.Order.Any(x => x.Field == field))
            {
                args.Args.RemoveAll(x => x.Field == field);
            }
