...
namespace YPHF.Core.Orm.SqlSugar
{
    ...
    public class SqlSugarFactory : IFactory, IDisposable
    {
                ...
                client.Aop.OnLogExecuted = (text, parameter) =>
                {
                    foreach (var p in parameter)
                    {
                        if (p.DbType == System.Data.DbType.Int16 || p.DbType == System.Data.DbType.Int32
                            || p.DbType == System.Data.DbType.Int64 || p.DbType == System.Data.DbType.Decimal
                            || p.DbType == System.Data.DbType.Double || p.DbType == System.Data.DbType.Single
                            || p.DbType == System.Data.DbType.UInt16 || p.DbType == System.Data.DbType.UInt32
                            || p.DbType == System.Data.DbType.UInt64 || p.DbType == System.Data.DbType.VarNumeric)
                        {
                            ...
                            else
                            {
                                switch (x.CommandType)
                                {
                                    ...
                                    case CommandEnum.Update:
                                        ...
                                        {
                                            client.Updateable(x.Model).IgnoreColumns(ignoreAllNullColumns: true).IsEnableUpdateVersionValidation().ExecuteCommand();
                                        }
                                        else
                                        {
                                            client.Updateable(x.Model).UpdateColumns(x.Where).IgnoreColumns(ignoreAllNullColumns: true).IsEnableUpdateVersionValidation().ExecuteCommand();
                                        }
                                        break;
        ...
        public void Delete<T>(string sql, object parameters = default!) where T : class, new()
            => commands.Add(new Command() { CommandType = CommandEnum.Delete, DataType = DataEnum.SQL, SQL = sql, Parameters = parameters });
        ...
        public IFastest<T> Fastest<T>() where T : class, new() => client.Fastest<T>();
        ...
        public void Insert<T>(string sql, object parameters = default!) where T : class, new()
            => commands.Add(new Command() { CommandType = CommandEnum.Insert, DataType = DataEnum.SQL, SQL = sql, Parameters = parameters });
        ...
        public ISugarQueryable<T> Query<T>() where T : class, new() => client.Queryable<T>();
        ...
        public void Update<T>(T model, Expression<Func<T, object>> columns) where T : class, new()
            => commands.Add(new Command() { CommandType = CommandEnum.Update, DataType = DataEnum.Where, Model = model, Where = columns });
        ...
        public void Update<T>(List<T> models, Expression<Func<T, object>> columns) where T : class, new()
            => commands.Add(new Command() { CommandType = CommandEnum.Update, DataType = DataEnum.Where, Model = models, Where = columns });
        ...
        public void Update<T>(string sql, object parameters = default!) where T : class, new()
            => commands.Add(new Command() { CommandType = CommandEnum.Update, DataType = DataEnum.SQL, SQL = sql, Parameters = parameters });
