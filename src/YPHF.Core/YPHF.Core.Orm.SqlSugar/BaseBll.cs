...
namespace YPHF.Core.Orm.SqlSugar
{
    ...
    public abstract class BaseBll<T> : IBaseBll<T> where T : IBaseModel, new()
    {
        ...
        public virtual async Task<BaseResult> AddAsync(T model) => await DAL.AddAsync(model);
        ...
        /// <param name="model">The model.</param>
        /// <returns>A Task&lt;BaseResult&gt; representing the asynchronous operation.</returns>
        public virtual async Task<BaseResult> EditAsync(T model) => await DAL.EditAsync(model);
        ...
        public virtual async Task<T> GetAsync(Expression<Func<T, bool>> where) => await DAL.GetAsync(where);
        ...
        public virtual async Task<List<T>> PageAsync(SearchArgs args) => await DAL.PageAsync(args);
    }
}